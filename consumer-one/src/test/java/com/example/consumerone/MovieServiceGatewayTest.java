...
public class MovieServiceGatewayTest {
    ...
    @Test
    public void getMoviesByInvalidScore() {

        float invalidScore = 11f;

        DslPart response = new PactDslJsonBody()//
                .numberType("status", 400)
                .stringType("error", HttpStatus.BAD_REQUEST.getReasonPhrase())
                .stringType("message", "Received invalid request. Score should be between 1 and 10.");

        RequestResponsePact pact = ConsumerPactBuilder//
                .consumer("consumer-one")//
                .hasPactWith("provider")//
                .given("Getting movies by score invalid score returns bad request")//
                .uponReceiving("get movies by invalid score")//
                .path("/movies")//
                .query("score=" + invalidScore)
                .method("GET")//
                .willRespondWith()//
                .status(400)//
                .body(response)//
        ...
        executeWithPact(pact, mockServer -> {
            config.setUrl(mockServer.getUrl());
            try {
                cut.getMoviesByScore(invalidScore);
            } catch (Exception e) {
                assertThat(e).isInstanceOf(HttpClientErrorException.class);
                assertThat(e.getMessage()).isEqualTo("400 Bad Request");
            }
        });
    }
